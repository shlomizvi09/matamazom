...
static ProductInfo findProductInfo(Matamazom matamazom, unsigned int id) {
    ASElement iterator = asGetFirst(matamazom->warehouse);
    if (iterator == NULL) {
        return NULL;
    }
    while (((ProductInfo) iterator)->id != id) {
        iterator = asGetNext(matamazom->warehouse);
        if (iterator == NULL) {
            return NULL;
        }
    }
    asGetFirst(matamazom->warehouse); //product's iterator back to start
    return iterator;
}
...
Matamazom matamazomCreate() {
    ...
    new_warehouse->warehouse = asCreate((ASElement) copyProductInfo,
                                        (ASElement) freeProduct,
                                        (ASElement) compareProductsID);
    if (new_warehouse->warehouse == NULL) {
        free(new_warehouse);
        return NULL;
    }
}
...
MatamazomResult mtmNewProduct(Matamazom matamazom,
    ...
    if (asContains(matamazom->warehouse, new_product)) {
        new_product->freeData(new_product->customData);
        free(new_product->name);
        free(new_product);
        return MATAMAZOM_PRODUCT_ALREADY_EXIST;
    }
    asRegister(matamazom->warehouse, new_product);
...
MatamazomResult mtmChangeProductAmount(Matamazom matamazom,
    ...
    AmountSetResult
            result = asChangeAmount(matamazom->warehouse, product_info, amount);
...
Order copyOrder(Order order) {
    ...
    new_order->cart = asCreate((ASElement) copyProductInfo, freeProduct,
                               compareProductsID);
    if (new_order->cart == NULL) {
        free(new_order);
        return NULL;
    }
    Node iterator_copy_from = asGetFirst(order->cart);
    Node iterator_copy_to = asGetFirst(new_order->cart);
    while (iterator_copy_from != NULL) {
        iterator_copy_to = malloc(sizeof(*iterator_copy_from));
        if (iterator_copy_to == NULL) {
            free(new_order->cart);
            free(new_order);
            return NULL;
        }
        iterator_copy_to->amount = iterator_copy_from->amount;
        iterator_copy_to->element = copyProductInfo(
                iterator_copy_from->element);
        iterator_copy_from = asGetNext(order->cart);
        iterator_copy_to = asGetNext(new_order->cart);
    }
